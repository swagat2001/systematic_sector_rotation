...
class TechnicalScorer:
    ...
    def _calculate_atr_signal(self, price_data: pd.DataFrame) -> float:
        """
        Calculate ATR signal for range expansion detection
        
        Returns signal from 0 to 1 indicating breakout potential
        """
        try:
            ...
            avg_atr = atr.iloc[-63:].mean()  # 3-month average
            
            # Signal if ATR expanding
            atr_ratio = safe_divide(current_atr, avg_atr, 1.0)
            
            if atr_ratio > 1.5:  # Significant expansion
                return 1.0
            elif atr_ratio > 1.2:
                return 0.7
            elif atr_ratio > 1.0:
